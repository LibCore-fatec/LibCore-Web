...
import { devolverLivroComToken } from "@/lib/servidor/servicos/libcore";
import { lerJson, textoObrigatorio } from "@/lib/servidor/validacoes/entrada";

export async function POST(requisicao: NextRequest) {
  try {
    const corpo = await lerJson(requisicao);
    return sucesso(await devolverLivroComToken(textoObrigatorio(corpo, "token_validacao"), textoObrigatorio(corpo, "etiqueta_rfid")));
  } catch (falha) {
